...
void LaserscanVirtualizer::virtual_laser_scan_parser()
{
  ...
  for (int i = 0; i < tokens.size(); ++i)
  {
    ...
    if (tfListener_.waitForTransform(base_frame, tokens[i], ros::Time(0), ros::Duration(1.0)))  // Check if TF knows the
                                                                                                // transform from this
                                                                                                // frame reference to
                                                                                                // base_frame reference
    {
  ...
  if ((tmp_output_frames.size() != output_frames.size()) ||
      !equal(tmp_output_frames.begin(), tmp_output_frames.end(), output_frames.begin()))
  {
...
LaserscanVirtualizer::LaserscanVirtualizer()
{
  ...
  point_cloud_subscriber_ = node_.subscribe<sensor_msgs::PointCloud2>(
      cloud_topic.c_str(), 1, boost::bind(&LaserscanVirtualizer::pointCloudCallback, this, _1));
  cloud_frame = "";
}
...
void LaserscanVirtualizer::pointcloud_to_laserscan(Eigen::MatrixXf points, pcl::PCLHeader scan_header,
                                                   int pub_index)  // pcl::PCLPointCloud2 *merged_cloud)
{
