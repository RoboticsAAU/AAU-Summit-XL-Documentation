...
// Load the controller
void GazeboRosPlanarMove::Load(physics::ModelPtr parent, sdf::ElementPtr sdf)
{
  parent_ = parent;
  ...
  {
    ROS_INFO_NAMED("planar_move",
                   "PlanarMovePlugin missing <enableYAxis>, "
                   "defaults to \"%d\"",
                   enable_y_axis_);
  }
  else
  ...
  {
    ROS_INFO_NAMED("planar_move",
                   "PlanarMovePlugin missing <disablePitchAndRoll>, "
                   "defaults to \"%d\"",
                   disable_pitch_and_roll_);
  }
  else
  {
  ...
  {
    ROS_WARN_NAMED("planar_move",
                   "PlanarMovePlugin (ns = %s) missing <publishTF>, "
                   "defaults to %d",
  ...
  if (!ros::isInitialized())
  {
    ROS_FATAL_STREAM_NAMED("planar_move", "PlanarMovePlugin (ns = " << robot_namespace_
                                                                    << "). A ROS node for Gazebo has not been "
                                                                       "initialized, "
                                                                    << "unable to load plugin. Load the Gazebo system "
                                                                       "plugin "
  ...
  ROS_DEBUG_NAMED("planar_move", "OCPlugin (%s) has started", robot_namespace_.c_str());
  ...
  ros::SubscribeOptions so = ros::SubscribeOptions::create<geometry_msgs::Twist>(
      command_topic_, 1, boost::bind(&GazeboRosPlanarMove::cmdVelCallback, this, _1), ros::VoidPtr(), &queue_);

  vel_sub_ = rosnode_->subscribe(so);
  odometry_pub_ = rosnode_->advertise<nav_msgs::Odometry>(odometry_topic_, 1);

  // start custom queue for diff drive
  callback_queue_thread_ = boost::thread(boost::bind(&GazeboRosPlanarMove::QueueThread, this));

  // listen to the update event (broadcast every simulation iteration)
  update_connection_begin_ =
      event::Events::ConnectWorldUpdateBegin(boost::bind(&GazeboRosPlanarMove::UpdateChildBegin, this));

  if (disable_pitch_and_roll_ == true)
  {
    ROS_WARN_NAMED("planar_move",
                   "PlanarMovePlugin (ns = %s) has <disablePitchAndRoll> to true, that means robot cannot move through "
                   "slopes",
                   robot_namespace_.c_str());

    update_connection_end_ =
        event::Events::ConnectWorldUpdateEnd(boost::bind(&GazeboRosPlanarMove::UpdateChildEnd, this));
  }
}
